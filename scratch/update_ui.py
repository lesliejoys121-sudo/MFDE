...
<body x-data="triageApp()" x-init="checkAuth()" x-cloak class="h-screen overflow-hidden flex flex-col">
    ...
    # Keep absolute python backend logic identical, only replace html content block inside root()
    new_app_content = re.sub(r'html_content = """(.*?)"""\n    return HTMLResponse\(content=html_content\)', f'html_content = """{new_html}"""\\n    return HTMLResponse(content=html_content)', content, flags=re.DOTALL)
